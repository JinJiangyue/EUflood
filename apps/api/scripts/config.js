...
exports.getPythonScriptDir = getPythonScriptDir;
exports.getUploadDir = getUploadDir;
exports.getGeoFileDir = getGeoFileDir;
exports.getOutputDir = getOutputDir;
exports.getPythonPath = getPythonPath;
exports.hasEmbeddedPython = hasEmbeddedPython;
exports.getEmbeddedPythonDir = getEmbeddedPythonDir;
exports.getPythonTimeout = getPythonTimeout;
...
function getPythonScriptDir() {
    ...
    if (process.env.PYTHON_SCRIPT_DIR) {
        ...
        if (path_1.default.isAbsolute(scriptDir)) {
            // 验证路径是否存在
            ...
            for (let i = 0; i < 6; i++) {
                ...
                if (dirName === 'apps' || currentDir.includes('apps' + path_1.default.sep) || currentDir.includes('apps\\')) {
                    // 找到 apps 目录，项目根目录是其父目录
                    projectRoot = path_1.default.dirname(currentDir);
                    break;
                }
            ...
            const resolvedDir = path_1.default.resolve(projectRoot, scriptDir);
    ...
    for (let i = 0; i < 6; i++) {
        const dirName = path_1.default.basename(currentDir);
        const parentDir = path_1.default.basename(path_1.default.dirname(currentDir));
    ...
    if (apiDir) {
        const scriptDir = path_1.default.join(apiDir, 'scripts');
        // 验证路径是否存在
        if (fs_1.default.existsSync(scriptDir)) {
    ...
    if (__dirname.includes('scripts') && !__dirname.includes('src')) {
        ...
        const apiDirFromDist = path_1.default.resolve(__dirname, '../../../');
        scriptDir = path_1.default.join(apiDirFromDist, 'scripts');
    }
    else {
        // 旧位置或未知：尝试从 apps/api 计算
        let currentDir = __dirname;
        let apiDirFromCurrent = null;
        for (let i = 0; i < 6; i++) {
            const dirName = path_1.default.basename(currentDir);
            const parentDir = path_1.default.basename(path_1.default.dirname(currentDir));
            if (dirName === 'api' && parentDir === 'apps') {
        ...
        if (apiDirFromCurrent) {
            scriptDir = path_1.default.join(apiDirFromCurrent, 'scripts');
        }
        else {
            // 最后回退：从项目根目录计算
            const projectRoot = path_1.default.resolve(__dirname, '../../../../..');
            scriptDir = path_1.default.join(projectRoot, 'apps', 'api', 'scripts');
        }
    }
    // 确保是绝对路径
    if (!path_1.default.isAbsolute(scriptDir)) {
        const projectRoot = path_1.default.resolve(__dirname, '../../../../..');
        scriptDir = path_1.default.resolve(projectRoot, scriptDir);
    }
...
function getUploadDir() {
    ...
    if (process.env.UPLOAD_DIR) {
        ...
        if (path_1.default.isAbsolute(uploadDir)) {
            // 确保目录存在
        ...
        // 相对路径：从项目根目录解析
        const projectRoot = path_1.default.resolve(__dirname, '../../../../..');
        const resolvedDir = path_1.default.resolve(projectRoot, uploadDir);
        if (!fs_1.default.existsSync(resolvedDir)) {
    ...
    let uploadDir;
    if (appsDir) {
        uploadDir = path_1.default.join(appsDir, 'uploads', 'rain_file');
    }
    else {
        // 回退方案：从项目根目录计算
        const projectRoot = path_1.default.resolve(__dirname, '../../../../..');
        uploadDir = path_1.default.join(projectRoot, 'apps', 'uploads', 'rain_file');
    }
    // 确保目录存在
    if (!fs_1.default.existsSync(uploadDir)) {
        fs_1.default.mkdirSync(uploadDir, { recursive: true });
 ...
 */
function getGeoFileDir() {
    // 如果环境变量指定了路径，使用环境变量
    if (process.env.GEO_FILE_DIR) {
        const geoFileDir = process.env.GEO_FILE_DIR;
        if (path_1.default.isAbsolute(geoFileDir)) {
            // 确保目录存在
            if (!fs_1.default.existsSync(geoFileDir)) {
                fs_1.default.mkdirSync(geoFileDir, { recursive: true });
            }
            return geoFileDir;
        }
        // 相对路径：从项目根目录解析
        const projectRoot = path_1.default.resolve(__dirname, '../../../../..');
        const resolvedDir = path_1.default.resolve(projectRoot, geoFileDir);
        if (!fs_1.default.existsSync(resolvedDir)) {
            fs_1.default.mkdirSync(resolvedDir, { recursive: true });
        }
        return resolvedDir;
    ...
    let geoFileDir;
    if (appsDir) {
        geoFileDir = path_1.default.join(appsDir, 'uploads', 'geofile');
    }
    else {
        // 回退方案：从项目根目录计算
        const projectRoot = path_1.default.resolve(__dirname, '../../../../..');
        geoFileDir = path_1.default.join(projectRoot, 'apps', 'uploads', 'geofile');
    }
    // 确保目录存在
    if (!fs_1.default.existsSync(geoFileDir)) {
        fs_1.default.mkdirSync(geoFileDir, { recursive: true });
...
function getOutputDir() {
    const outputDir = process.env.OUTPUT_DIR || path_1.default.join(__dirname, '../../../outputs');
    // 如果是相对路径，转换为绝对路径
    if (!path_1.default.isAbsolute(outputDir)) {
        const projectRoot = path_1.default.resolve(__dirname, '../../../../..');
        return path_1.default.resolve(projectRoot, outputDir);
    }
    // 确保目录存在
...
function getPythonPath() {
    ...
    if (process.env.PYTHON_PATH) {
        ...
        if (!path_1.default.isAbsolute(envPath)) {
            const projectRoot = path_1.default.resolve(__dirname, '../../../../..');
            const resolvedPath = path_1.default.resolve(projectRoot, envPath);
    ...
    const cwd = process.cwd();
    let embedPythonPath;
    let embedPythonPathUnix;
    // 方式1: 从当前工作目录（apps/api）直接计算
    if (cwd.endsWith('apps/api') || cwd.includes('apps\\api')) {
        embedPythonPath = path_1.default.join(cwd, 'python-embed', 'python.exe');
        embedPythonPathUnix = path_1.default.join(cwd, 'python-embed', 'python');
        if (fs_1.default.existsSync(embedPythonPath)) {
            console.log(`[Python] ✓ Found embedded Python (from cwd): ${embedPythonPath}`);
            return embedPythonPath;
        }
    }
    // 方式2: 从 __dirname 计算
    let projectRoot = path_1.default.resolve(__dirname, '../../../../..');
    if (projectRoot.includes('dist')) {
        projectRoot = path_1.default.resolve(projectRoot, '..');
    }
    embedPythonPath = path_1.default.join(projectRoot, 'python-embed', 'python.exe');
    embedPythonPathUnix = path_1.default.join(projectRoot, 'python-embed', 'python');
    console.log(`[Python] Checking embedded Python:`);
    console.log(`[Python]   - From cwd: ${cwd.includes('apps/api') ? path_1.default.join(cwd, 'python-embed', 'python.exe') : 'N/A'}`);
    ...
    if (process.platform === 'win32') {
        ...
        if (fs_1.default.existsSync(embedPythonPathUnix)) {
...
function hasEmbeddedPython() {
    const projectRoot = path_1.default.resolve(__dirname, '../../../../..');
    if (process.platform === 'win32') {
        const embedPythonPath = path_1.default.join(projectRoot, 'python-embed', 'python.exe');
        return fs_1.default.existsSync(embedPythonPath);
    }
    else {
        const embedPythonPath = path_1.default.join(projectRoot, 'python-embed', 'python');
        return fs_1.default.existsSync(embedPythonPath);
    }
}
/**
...
function getEmbeddedPythonDir() {
    const projectRoot = path_1.default.resolve(__dirname, '../../../../..');
    const embedDir = path_1.default.join(projectRoot, 'python-embed');
    if (fs_1.default.existsSync(embedDir)) {
        return embedDir;
    }
    return null;
}
