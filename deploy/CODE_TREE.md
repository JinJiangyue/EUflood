...
│   ├── main.js          主入口文件（初始化所有模块、路由、状态管理）
│   ├── core/           核心模块目录 ⭐ v1.0.7 新增
│   │   ├── router.js    客户端路由系统（SPA路由、页面切换）
│   │   └── state-manager.js  全局状态管理器（查询状态、结果缓存）
│   └── modules/
│       ├── stats.js             统计数据加载
│       ├── events.js            事件管理（查询、详情、深度搜索、分页）
│       ├── global-query.js      全局查询模块 ⭐ v1.0.7 新增
│       ├── map.js               地图管理（初始化、GeoJSON、数据点）
│       └── interpolation.js     空间插值分析（文件上传、处理）
└── index.html           多页面架构主HTML（侧边栏导航、页面容器）⭐ v1.0.7 重构
```

### 主要功能
- **多页面架构（SPA）**：⭐ v1.0.7 重构
  - 侧边栏导航：总览仪表盘、事件查询、空间插值分析、数据分析、数据管理
  - 客户端路由：基于 hash 的路由系统，无需刷新页面切换
  - 页面容器：每个功能模块独立页面容器，按需显示
- **全局查询系统**：⭐ v1.0.7 新增
  - 侧边栏全局查询：日期范围、国家筛选
  - 快速日期选择：最新、今天、本周、本月
  - 状态管理：全局状态管理器，统一管理查询结果
  - 跨页面共享：查询结果在所有页面间共享
- **模块化设计**：按功能拆分为独立模块，易于维护和扩展
- **零构建**：使用原生 JavaScript，无需构建工具
- **静态文件服务**：通过 Express 静态文件中间件提供 CSS/JS
- **事件查询页面**：
  - 快速日期选择按钮（最新、今天、本周、本月）
  - 默认显示最新10条事件（最近30天）
  - 分页功能：每页条数可配置（10/20/50/100）
  - 查询结果提示：成功/失败/无数据提示
- **空间插值分析界面**：
  - 文件上传功能
  - 阈值可配置（默认 50.0，可修改）
...
- **i18n.js**：国际化模块
  ...
  - 提供 `t()` 函数获取翻译文本（支持参数替换）
  - 提供 `setLanguage()` 函数切换语言
  - 自动更新页面上的所有 `data-i18n` 元素
  - 语言选择保存到 localStorage
  - 语言切换器 UI（右上角按钮）
- **core/router.js**：客户端路由系统 ⭐ v1.0.7 新增
  - 基于 hash 的路由（`#dashboard`, `#events`, `#interpolation` 等）
  - 页面切换：自动显示/隐藏页面容器
  - 导航状态：自动更新侧边栏导航的 active 状态
  - 页面事件：触发 `page:show` 事件，便于模块初始化
- **core/state-manager.js**：全局状态管理器 ⭐ v1.0.7 新增
  - 查询状态：管理查询参数、结果、加载状态
  - 状态订阅：支持监听状态变化
  - 事件通知：通过 `globalQuery:updated` 事件通知状态变化
- **main.js**：主入口文件
  - 初始化国际化、路由、全局查询、各功能模块
  - 监听路由变化和全局查询更新事件
  - 地图延迟初始化（仅在空间插值页面）
- **stats.js**：统计数据加载和显示
- **events.js**：事件查询、候选列表、详情展示、深度搜索功能
  - 支持表1/表2数据条件显示
  - 深度搜索按钮和确认对话框
  - 深度搜索进度显示和错误处理
  - 超时处理（4分钟）
